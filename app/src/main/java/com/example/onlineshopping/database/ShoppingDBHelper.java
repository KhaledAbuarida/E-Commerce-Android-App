...
public class ShoppingDBHelper extends SQLiteOpenHelper {
    ...
    public List<Category> getAllCategory ( ){
        ...
        Category c=null ;        List<Category> arrayList = new ArrayList();
        while (res.moveToNext()) {
...
//        ContentValues cv1 = new ContentValues();
        ...
        long result = db.update("Category", cv, "categoryId=?",new String[]{categoryID});
        db.close();
   ...
   public void deleteOneProduct(String productID){
    ...
    public void deleteOneCategory(String categoryID){
        SQLiteDatabase db = this.getWritableDatabase();
        long result = db.delete("Category","categoryID=?",new String[]{categoryID});
        if(result == -1){
            System.out.println("FAILED TO UPDATE!!!!!!!!!");
            Toast.makeText(context, "Failed to Delete", Toast.LENGTH_SHORT).show();
        }else {
